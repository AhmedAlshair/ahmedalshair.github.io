<script lang="ts">
  ...
  import CometLogoIcon from '@assets/works/company-icons/comet-logo-icon.svg?raw';
  import EnactLogoIcon from '@assets/works/company-icons/enact-logo-icon.svg?raw';
  import TTPLogoIcon from '@assets/works/company-icons/ttp-logo.svg?raw';
  ...
  const itemDetails = [
    ...
    },
    {
      ...
      icon: TTPLogoIcon,
      conciseText:
        'Delivered high-impact interactive experiences for global healthcare leaders including Pfizer, Merck, and Abbott. Managed the full design-to-code workflow for CLM applications, balancing strict pharmaceutical compliance with advanced animations and storytelling.',
      responsibilities: [
...
{#snippet TimelineItem(
  ...
  >
    ...
    <div class="item-content">
      ...
      <h4 class="item-title flex-layout gap gap-xs align-center justify-start">
        {#if concise}
      ...
      {#if !concise}
        ...
        >
          <summary class={['flex-layout align-center', concise && 'disabled']}>
            <span>{item.company}</span>
            <Icon name="keyboard_arrow_down" className="icon" />
          </summary>
...
<style>
  ...
  .timeline {
    ...
    .item-date {
      ...
      .vertical & {
        line-height: calc(var(--space-2xl) + 1rem);
      }
    }
    ...
    .concise {
      ...
      color: var(--text-muted);
      border-left: var(--stroke-width-thin) solid var(--border-color);
      transition:
        background-color var(--transition-default),
        color var(--transition-default);
    }
